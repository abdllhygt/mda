defmodule Mda do
  def to_html(mda_source) do
    ...
    html_content = for block <- mda_blocks  do
  ...
  defp h(block) do
    ...
    {title, info} = if String.contains?(unsigned_block, "\n") do
  ...
  defp image(block) do
    ...
    {link_text, alt_text} = case String.split(replaced_block, "]") do
  ...
  defp ol(block) do
    ...
    li_lines = for line <- String.split(replaced_block, "\n") do
      "<li>#{line}</li>"
    end
  ...
  defp alert(block) do
    {b_type, title, content, icon} = cond do
  ...
  defp table(block) do
    ...
    content = for cb <- content_blocks do
      rows = for row <- String.split(cb, "|") do
        "<td>#{to_span row}</td>\n"
      end
      "<tr>#{rows}</tr>\n"
    end
  ...
  defp sentence(block)  do
    ...
    sentence_tag = if String.starts_with?(first, ">[") do
    ...
    subsentence_tags = for i <- other_parts do
      """
  ...
  def to_span(text) do
    replaced_text = Regex.replace ~r/s\[\w*\]s/u, text, fn x, _ ->
